...
func keyExpansionCore(in []byte, i byte) {
...
func keyExpansion(inputKey []byte, expandedKey []byte) {
...
func subBytes(state [BLOCK_SIZE]byte) {
    for i := 0; i < BLOCK_SIZE; i++ {
       state[i] = sBox(state[i])
    }
}

func shiftRows(state [BLOCK_SIZE]byte) {
    var tmp [BLOCK_SIZE]byte
...
func mixColumns(state [BLOCK_SIZE]byte){
    mul2 := []byte{
...
func addRoundKey(state [BLOCK_SIZE]byte, key []byte) {
    for i := 0; i < BLOCK_SIZE; i++ {
       state[i]  ^=  key[i]
    }
}

func round(state [BLOCK_SIZE]byte, roundKey []byte) {
    subBytes(state)
    shiftRows(state)
    mixColumns(state)
...
func finalRound(state [BLOCK_SIZE]byte, roundKey []byte) {
    subBytes(state)
    shiftRows(state)
    addRoundKey(state, roundKey)
}
...
func encrypt(message []byte, key []byte) {
    ...
    var state [BLOCK_SIZE]byte
    for i := 0; i < BLOCK_SIZE; i++ {
        state[i] = message[i]
    }

    var numRounds int = 1
    keyExpansion(key, byte(numRounds))
